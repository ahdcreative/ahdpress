<article id="post-<?php the_ID(); ?>" <?php post_class('mb-12'); ?>>
    ...
    <?php else : ?>
        <div class="entry-content">
            <?php
            /* Translators: %s: The name of the current post. */
            the_content(
                sprintf(
                    __('Continue reading %s', 'ahdpress'),
                    the_title('<span class="screen-reader-text">"', '"</span>', false)
                )
            );
            wp_link_pages(
                array(
                    'before' => '<div class="page-links"><span class="page-links-title">' . __('Pages:', 'ahdpress') . '</span>',
                    'after' => '</div>',
                    'link_before' => '<span>',
                    'link_after' => '</span>',
                    'pagelink' => '<span class="screen-reader-text">' . __('Page', 'ahdpress') . ' </span>%',
